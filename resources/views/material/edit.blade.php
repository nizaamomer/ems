        ...
        <nav class="flex" aria-label="Breadcrumb">
            <ol class="inline-flex items-center ">
                ...
                <div class="flex items-center">
                    ...
                    <p href="#" class="text-sm font-medium text-gray-700  md:ml-2 dark:text-indigo-400 ">
دەستکاریکردنی مادە</p>
                </div>
                </li>
    ...
    <div class="lg:w-9/12 mx-auto   my-5 flex items-center justify-between mb-4">
        <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-indigo-400">دەستکاریکردنی مادە</h5>
        <a href="{{ url()->previous() }}"
    ...
    <div
        class="w-full lg:w-9/12 mx-auto p-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
        <div class="flow-root">
            <form method="POST" action="{{ route('material.update', $material->id) }}">
                ...
                @method('PUT')
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="name" id="name"
                            ...
                            placeholder=" " value="{{ $material->name }}" />
                        <label for="name"
                    ...
                    <div class="relative z-0 w-full mb-6 group">
                        <input type="text" name="code" id="code"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-indigo-500 focus:outline-none focus:ring-0 focus:border-indigo-600 peer"
                            placeholder=" " value="{{ $material->code }}" />
                        <label for="code"
                            class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:right-0 peer-focus:text-indigo-600 peer-focus:dark:text-indigo-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 after:content-['*'] after:ml-0.5 after:text-red-500 ">کۆدی مادە</label>
                        @error('code')
                            <div class="mt-1 text-sm text-red-400">{{ $message }}</div>
                ...
                <div class="text-right">
                    <button type="submit"
                    class="dark:text-black text-white mt-5 bg-indigo-700  focus:outline-none  font-medium rounded text-sm w-full sm:w-1/2 px-5 py-2 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">تازەکردنەوەی مادە</button>
                </div>
            </form>
        </div>
    </div>
